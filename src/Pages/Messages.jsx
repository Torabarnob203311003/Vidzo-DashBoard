...
const SOCKET_URL =
  import.meta.env.VITE_SOCKET_URL || "http://98.95.167.179:6002";
...
const getFileIcon = (type = "") => {
  if (type.startsWith("image/"))
    return { icon: ImageIcon, color: "#3B82F6", bg: "#EFF6FF" };
  if (type.startsWith("video/"))
    return { icon: Film, color: "#8B5CF6", bg: "#F5F3FF" };
  if (type.startsWith("audio/"))
    return { icon: Music, color: "#EC4899", bg: "#FDF2F8" };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div
          className={`text-sm font-semibold truncate ${isMe ? "text-white" : "text-gray-800"}`}
        >
          {file?.name || file?.originalName || "File"}
        </div>
        <div
          className={`text-xs mt-0.5 ${isMe ? "text-white/70" : "text-gray-400"}`}
        >
      ...
      >
        {images.length > 1 && (
          <>
            <button
              onClick={onPrev}
              className="absolute left-4 p-2.5 rounded-full bg-white/10 hover:bg-white/25 text-white transition z-10"
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={onNext}
              className="absolute right-4 p-2.5 rounded-full bg-white/10 hover:bg-white/25 text-white transition z-10"
            >
          ...
          {images.map((im, i) => (
            <button
              ...
              className={`w-12 h-12 rounded-lg overflow-hidden border-2 transition ${
                i === currentIndex ? "scale-110" : "opacity-40 hover:opacity-70"
              }`}
...
const DropOverlay = ({ active }) => (
  ...
  >
    <div className="text-center">
      ...
      <div className="text-base font-bold" style={{ color: GOLD_DARK }}>
        Drop files to send
      </div>
...
const Messages = () => {
  ...
  // Socket live state
  const [liveMessages, setLiveMessages] = useState({}); // conversationId -> msg[]
  const [onlineUsers, setOnlineUsers] = useState([]); // array of userIds
  // typingUsers: conversationId -> { senderId, senderRole }
  ...
  // Merge API + live socket messages, deduplicate by _id
  ...
  const newSocketMessages = socketMessages.filter(
    (m) => !apiMessageIds.has(m._id),
  );
  const currentMessages = [...apiMessages, ...newSocketMessages];

  // All image messages for lightbox
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      // ✅ Announce admin joined — backend listens: 'support_user_join'
      // Replace "ADMIN_ID" with actual admin ID from your auth store if available
      const adminId = "admin"; // TODO: replace with real admin ID from auth
      socket.emit("support_user_join", { userId: adminId, role: "admin" });
    });
    ...
    // ✅ Backend emits: 'support_new_message'
    ...
    // ✅ Backend emits: 'support_user_online' with { userId, role }
    ...
    // ✅ Backend emits: 'support_user_offline' with { userId }
    socket.on("support_user_offline", ({ userId }) => {
      setOnlineUsers((prev) => prev.filter((id) => id !== userId));
    });

    // ✅ Backend emits: 'support_user_typing' with { conversationId, senderId, senderRole }
    socket.on(
      "support_user_typing",
      ({ conversationId, senderId, senderRole }) => {
        // Only show typing for non-admin senders (users typing to admin)
        if (senderRole !== "admin") {
          setTypingUsers((prev) => ({
            ...prev,
            [conversationId]: { senderId, senderRole },
    ...
    // ✅ Backend emits: 'support_user_stop_typing' with { conversationId, senderId }
    ...
    // ✅ Backend emits: 'support_messages_read' — update read state
    socket.on("support_messages_read", ({ conversationId }) => {
    ...
    // ✅ Backend emits: 'support_status_changed'
    // eslint-disable-next-line no-unused-vars
    socket.on("support_status_changed", ({ conversationId, status }) => {
      refetchConversations();
    });

    // ✅ Backend emits: 'support_notification' — show toast
    socket.on("support_notification", ({ type, message }) => {
      if (type === "new_message") {
        toast.info(message || "New support message");
      }
    });
  ...
  useEffect(() => {
    ...
    // ✅ Backend listens: 'support_join_conversation'
    socketRef.current.emit(
      "support_join_conversation",
      selectedConversation._id,
    );

    // Mark messages as read via API
    makeReadAllMessage(selectedConversation._id)
      .then(() => {
        // ✅ Also notify via socket — backend listens: 'support_mark_read'
    ...
    return () => {
      // Socket.io rooms are auto-cleaned on disconnect; no explicit leave needed
      // but you can emit a leave event if your backend supports it
    };
  }, [selectedConversation?._id]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (replyingTo) {
        formData.append("replyToId", replyingTo);
      }
      ...
      if (res?.data?.success) {
        ...
        // ✅ Broadcast via socket — backend listens: 'support_send_message'
        // The backend will save it AND broadcast 'support_new_message' to the room
        // Since we already saved via API, we just need to add to local state
        // to avoid duplicates we rely on the deduplication logic above
        const convId = selectedConversation._id;
        setLiveMessages((prev) => ({
          ...prev,
          [convId]: [...(prev[convId] || []), sentMessage],
        }));
  ...
  const handleTyping = (e) => {
    ...
    // ✅ Backend listens: 'support_typing'
    socketRef.current.emit("support_typing", {
      conversationId: selectedConversation._id,
      senderId: "admin", // TODO: replace with real admin ID
      senderRole: "admin",
    });

    clearTimeout(typingTimeoutRef.current);
    typingTimeoutRef.current = setTimeout(() => {
      // ✅ Backend listens: 'support_stop_typing'
      socketRef.current?.emit("support_stop_typing", {
        conversationId: selectedConversation._id,
        senderId: "admin",
      });
  ...
  // Is the user (not admin) typing in this conversation?
  ...
  return (
    <div
      ...
      className="flex h-[85vh] bg-gray-50 overflow-hidden"
    >
      {/* ── Sidebar ── */}
              ...
              <>
                ...
                <div className="space-y-1">
                  {currentMessages.map((msg) => {
                    ...
                    const isImageMsg =
                      hasFile && msg.file?.mimeType?.startsWith("image/");
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`flex flex-col ${isMe ? "items-end" : "items-start"} max-w-xs lg:max-w-sm`}
                        >
                          {/* Reply preview */}
                          {replyTarget && (
                            <button
                            ...
                            >
                              ...
                              >
                                {isAdminMessage(replyTarget)
                                  ? "You"
                                  : selectedConversation.user?.name}
                              </div>
                              <div className="text-xs text-gray-600 truncate">
                                {replyTarget.file?.url
                                  ? `📎 ${replyTarget.file?.name || "File"}`
                                  : replyTarget.message}
                              </div>
                            </button>
                          )}
                          ...
                          <div className="relative">
                            ...
                            {isImageMsg && (
                              ...
                              >
                                <ImageBubble
                                  file={msg.file}
                                  onPreview={() => openLightbox(msg._id)}
                                />
                              </div>
                            )}
                  ...
                  {/* Typing indicator — show when user (not admin) is typing */}
                  {isUserTyping && (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    Replying to{" "}
                    {isAdminMessage(
                      currentMessages.find((m) => m._id === replyingTo),
                    )
                      ? "yourself"
                      : selectedConversation.user?.name}
                  </div>
              ...
              <div className="mb-3 p-3 rounded-xl bg-gray-50 border border-gray-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold text-gray-500">
                    ...
                    {pendingFiles.length > 1 ? "s" : ""} to send
                  </span>
                  <button
        ...
        <Lightbox
          ...
          onNext={(i) =>
            setLightbox((l) => ({
              ...l,
              index:
                typeof i === "number" ? i : (l.index + 1) % l.images.length,
            }))
          }
        />
      )}
