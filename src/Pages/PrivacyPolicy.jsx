...
const PrivacyPolicy = () => {
  ...
  const { data, isLoading } = useGetDocumentationQuery("privacyPolicy");
  ...
  const onSubmit = async () => {
    try {
      ...
      if (data?.data?.content) {
        ...
        if (res?.error) {
          return toast.error(
            res.error.data?.message || "Privacy Policy update failed",
          );
        }
    ...
    <div className="p-10 bg-[#F8FAFC] min-h-screen">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {(!data?.data?.content && (
          ...
          >
            {isCreating ? "Creating..." : "Create Privacy Policy"}
          </button>
        )) || (
          <button
            type="submit"
